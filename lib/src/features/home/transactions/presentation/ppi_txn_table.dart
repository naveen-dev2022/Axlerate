...
class PpiTxnTableWidget extends ConsumerStatefulWidget {
  PpiTxnTableWidget({
    super.key,
  ...
  bool showDateFilter;
  late PpiTxnQueryParams? txnParams;
  final bool showPagination;
  String userOrgEnrollId;
  // final StateNotifierProvider<PageNotifierNew, PaginatorModel>? stateNotifier;
  @override
  ConsumerState<ConsumerStatefulWidget> createState() => _PpiTxnTableWidgetState();
}

class _PpiTxnTableWidgetState extends ConsumerState<PpiTxnTableWidget> {
  @override
  void initState() {
    widget.txnParams ??= PpiTxnQueryParams(
      size: 15,
      pageIndex: 1,
    );
  ...
  Future<void> getTransactionsList(PpiTxnQueryParams params) async {
    Future(
      () async {
        ...
        ref.read(ppiTransactionListStateProvider.notifier).state = null;
        ref.read(ppiTransactionListStateProvider.notifier).state =
            await ref.read(transactionControlProvider).listPpiTxns(params: params, orgEnrollIdOfUser: userOrgEnrollId);
      },
    );
  }

  List<String> headerItems = ["Date", "Description", "Amount", "User"];
  ...
  Widget build(BuildContext context) {
    ...
    final txnList = ref.watch(ppiTransactionListStateProvider);

    return ppiTxnTableWidget(txnList, screenHeight);
  }

  Widget ppiTxnTableWidget(PpiTxnListModel? txnList, double screenHeight) {
    return Column(
      children: [
                        ...
                        SizedBox(
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (BuildContext context, int index) {
                              ...
                              Doc item = txnList.data!.message!.docs![index]!;
                              ...
                              List<String> itemRowData = [
                                ...
                                item.metadata!.accountInfoEntity! == item.metadata!.userEntityId
                                    ? item.user!.name!
                                    : '',
                                '${item.organization!.firstName!} ${item.organization!.lastName!}',
                                '${item.partnerOrganization!.firstName!} ${item.partnerOrganization!.lastName!}',
                              ];
