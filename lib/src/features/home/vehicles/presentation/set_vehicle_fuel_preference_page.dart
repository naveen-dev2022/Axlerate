...
@RoutePage()
class SetVehicleFuelPreferencePage extends ConsumerStatefulWidget {
  const SetVehicleFuelPreferencePage({
    super.key,
    @PathParam('custId') required this.orgEntityId,
  ...
  @override
  ConsumerState<ConsumerStatefulWidget> createState() => _SetOrgFuelPreferencePageState();
}

class _SetOrgFuelPreferencePageState extends ConsumerState<SetVehicleFuelPreferencePage> {
  ...
  bool isMobile = false;
  Vehicle? vehicle;
  Service? vehicleServiceFuel;
  FuelLimitResponseModel? limitResponse;

  getVehicleFuelLimit() async {
    await ref.read(vehicleControllerProvider).getVehicleByRegistrationNumber(vehicleEnrolId: widget.vehicleRegNo);
    vehicle = ref.watch(vehicleDetailsProvider);
    vehicleServiceFuel = getVehicleService(vehicle, "FUEL");

    limitResponse = await ref.read(logisticsControllerProvider).getFuelLimitByEntityType(
          entityId: vehicleServiceFuel?.vehicleEntityId ?? '',
          entityType: 'VEHICLE',
          orgEnrollId: widget.orgEntityId,
          issuerName: 'HPCL',
          vehicleRegistrationNumber: widget.vehicleRegNo.toUpperCase(),
        );
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: Column(
          ...
          children: [
            Text(HomeConstants.vehicleFuelCardPreference, style: AxleTextStyle.titleMedium),
                        ...
                        children: [
                          SizedBox(
                            ...
                            child: CardServiceToggleWidget(
                              ...
                              defaultCardTitle: "Set Fuel Limit For Vehicle",
                              secondaryCardTitle: "Set Wallet Limit For Vehicle",
                          ...
                          //     defaultCardTitle: "Set Wallet Limit For Vehicle",
                          //     secondaryCardTitle: "Set Fuel Limit For Vehicle",
                          ...
                          //     onChange: (val) async {
                          //       setState(() => ismonthlyLimitEnabled = !ismonthlyLimitEnabled);
                          //     },
                          //   ),
                          // ),
                          // const SizedBox(height: defaultPadding),
                  ...
                  AxlePrimaryButton(
                      ...
                      onPress: () async {
                        ...
                        await ref
                            ...
                            .setVehicleFuelLimit(formInput: getInputs(), vehicleEnrollId: widget.vehicleRegNo);
  ...
  SetVehicleFuelInputModel getInputs() {
    return SetVehicleFuelInputModel(
      organizationEnrollmentId: widget.orgEntityId,
      issuerName: 'HPCL',
      fuelLimit: VehicleLimit(
        daily: int.parse(fuelDailyLimitController.text),
        // monthly: int.parse(fuelMonthlyLimitController.text),
        // quarterly: int.parse(fuelQuarterlyLimitController.text),
        // halfYearly: int.parse(fuelHalfYearlyLimitController.text),
        // yearly: int.parse(fuelYearlyLimitController.text),
        // dailyTransactionCount: int.parse(fuelDailyTransactionCountController.text),
        // monthlyTransactionCount: int.parse(fuelMonthlyTransactionCountController.text),
        // quarterlyTransactionCount: int.parse(fuelQuarterlyTransactionCountController.text),
        // halfYearlyTransactionCount: int.parse(fuelHalfYearlyTransactionCountController.text),
        // baseLimit: int.parse(fuelDailyLimitController.text),
      ),
      // walletLimit: VehicleLimit(
      //   daily: int.parse(walletDailyLimitController.text),
      //   monthly: int.parse(walletMonthlyLimitController.text),
      //   quarterly: int.parse(walletQuarterlyLimitController.text),
      //   halfYearly: int.parse(walletHalfYearlyLimitController.text),
      //   yearly: int.parse(walletYearlyLimitController.text),
      //   dailyTransactionCount: int.parse(walletDailyTransactionCountController.text),
      //   monthlyTransactionCount: int.parse(walletMonthlyTransactionCountController.text),
      //   quarterlyTransactionCount: int.parse(walletQuarterlyTransactionCountController.text),
      //   halfYearlyTransactionCount: int.parse(walletHalfYearlyTransactionCountController.text),
      // ),
    );
  }
}
