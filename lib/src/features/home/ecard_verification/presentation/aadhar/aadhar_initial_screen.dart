...
@RoutePage()
class AadharScreenInitial extends ConsumerStatefulWidget {
  const AadharScreenInitial({Key? key}) : super(key: key);

  @override
  ConsumerState<AadharScreenInitial> createState() =>
      _AadharScreenInitialState();
}

class _AadharScreenInitialState extends ConsumerState<AadharScreenInitial> {
  bool isMobile = false;
      ...
      body: Column(
        children: [
          DynamicVerificationCard(
            onTap: () {
              context.router.pushNamed(RouteUtils.getAadhaarHistoryPath());
            },
            imageUrl: 'assets/images/aadhar_big_initial.svg',
          ),
          Expanded(
            child: Container(
  ...
  Future validateAadhar() async {
    if (_formKey.currentState!.validate()) {
      setState(() {
        showValidationError = false;
      });
      _loader.show(context);
      ref.read(aadhaarOtpStateProvider.notifier).state = null;
      await ref
          ...
          .then(
        (value) {
          ref.read(aadhaarOtpStateProvider.notifier).state = value;
          final aadharDataList = ref.watch(aadhaarOtpStateProvider);
          _loader.hide();
          if (aadharDataList?.status == true) {
            context.router.pushNamed(RouteUtils.getAadhaarOtpPath());
            return null;
          } else {
